...
class Digraph
{
    ...
    private:
        ...
        mutable std::map<int, DigraphVertex<VertexInfo, EdgeInfo>> graph;
        mutable std::map<int, bool> visited;
        mutable int visitedCount;

        void DFT() const;
        void DFTr(int vertex) const;

    public:
        std::map<int, int> findShortestPaths(int startVertex) const;
};
...
{
    std::vector<int> vec;
    for (auto E : graph) vec.push_back(E.first);
    return vec;
}
...
std::vector<std::pair<int, int>> Digraph<VertexInfo, EdgeInfo>::edges() const
{
    ...
    for (auto E : graph)
        for (auto F : E.second.edges)
            vec.push_back(std::make_pair(F.fromVertex, F.toVertex));
    return vec;
}
...
{
    if (graph.count(vertex) == 0) throw DigraphException("vertex does not exist");
    std::vector<std::pair<int, int>> vec;
    for (auto E : graph.at(vertex).edges)
        vec.push_back(std::make_pair(E.fromVertex, E.toVertex));
    return vec;
}
...
EdgeInfo Digraph<VertexInfo, EdgeInfo>::edgeInfo(int fromVertex, int toVertex) const
{
    if (graph.count(fromVertex) == 0 || graph.count(toVertex) == 0)
        throw DigraphException("one or more of the vertices do not exist");

    for (auto E : graph.at(fromVertex).edges)
        if (fromVertex == E.fromVertex && toVertex == E.toVertex)
            return E.einfo;
...
{
    if (graph.count(vertex) == 1) throw DigraphException("vertex already exists");
...
void Digraph<VertexInfo, EdgeInfo>::addEdge(int fromVertex, int toVertex, const EdgeInfo& einfo)
{
    ...
    graph[fromVertex].edges.push_back(temp);
}
...
void Digraph<VertexInfo, EdgeInfo>::removeVertex(int vertex)
{
    ...
    for (auto E : graph)
    {
        std::list<DigraphEdge<EdgeInfo>> temp;
        for (auto F : E.second.edges)
            if (F.toVertex != vertex) temp.push_back(F);
        E.second.edges.clear();
        E.second.edges = temp;
    }
}
...
void Digraph<VertexInfo, EdgeInfo>::removeEdge(int fromVertex, int toVertex)
{
    ...
    int oldSize = graph.at(fromVertex).edges.size();
    std::list<DigraphEdge<EdgeInfo>> temp;

    for (auto E : graph.at(fromVertex).edges)
    {
        if (E.fromVertex == fromVertex && E.toVertex == toVertex) continue;
        else temp.push_back(E);
    }

    graph.at(fromVertex).edges.clear();
    graph.at(fromVertex).edges = temp;

    if (graph.at(fromVertex).edges.size() == oldSize)
        throw DigraphException("edge does not exist");
}
...
{
    for (auto E : graph)
        visited.at(E.first) = false;

    for (auto E : graph)
    {
        visitedCount = 0;
        DFTr(E.first);
        if (visitedCount != graph.size()) return false;
    }
...
template<typename VertexInfo, typename EdgeInfo>
std::map<int, int> Digraph<VertexInfo, EdgeInfo>::findShortestPaths(int startVertex,
        std::function<double(const EdgeInfo&)> edgeWeightFunc) const
{
    // used my own findShortestPaths() with my own PriorityQueue(). 
    // did not use edgeWeightFunc
...
// ===========================================================================
// Digraph Priority Queue

class PriorityQueue
{
    public:
        PriorityQueue() { m.clear(); }

        void enqueue(int vertex, double distance)
        {
            m[distance] = vertex;
        }
    ...
    private:
        // I chose map because it automatically sorts the contents based on the key 
        // priority queue using map as the underlying container has logarithmic enqueue time
        // dequeue time is constant in this case, because it only removes the first element
...
template<typename VertexInfo, typename EdgeInfo>
void Digraph<VertexInfo, EdgeInfo>::DFT() const
{
    for (auto E : graph)
        visited.at(E.first) = false;

    for (auto E : graph)
        if (!visited.at(E.first)) DFTr(E.first);
}

template<typename VertexInfo, typename EdgeInfo>
void Digraph<VertexInfo, EdgeInfo>::DFTr(int vertex) const
{
    visited.at(vertex) = true;
    ++visitedCount;

    for (auto E : graph.at(vertex).edges)
        if (!visited.at(E.toVertex)) DFTr(E.toVertex);
}

template<typename VertexInfo, typename EdgeInfo>
std::map<int, int> Digraph<VertexInfo, EdgeInfo>::findShortestPaths(int startVertex) const
{
    std::map<int, bool> k; // vertex known or unknown
    std::map<int, int> p; // vertex and its predecessors
    std::map<int, double> d; // distance to vertex
    ...
    for (auto E : graph)
    {
        k[E.first] = false;
        p[E.first] = E.first;
        d[E.first] = E.first == startVertex ? 0.0 : std::numeric_limits<double>::max();
    }
    ...
    while (!pq.empty())
    {
        ...
        if (k.at(v) == false)
        {
            ...
            for (auto E : graph.at(v).edges)
            {
                if (d.at(E.toVertex) > d.at(v) + E.einfo.miles)
                {
                    d.at(E.toVertex) = d.at(v) + E.einfo.miles;
                    p.at(E.toVertex) = v;
                    pq.enqueue(E.toVertex, E.einfo.miles);
                }
            }
        }
    }
